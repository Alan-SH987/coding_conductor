...
def test_cold_start_fresh_db(tmp_path):
    ...
    assert {"is_pinned", "is_archived", "deleted_at", "quota_tokens", "quota_cost_usd", "verify_cmd", "enabled_skills", "auto_heal_rounds"} <= cols
    assert {"deleted_at", "error", "tags"} <= _columns(db, "task")
    assert _alembic_version(db) == ["009"]
