...
function DiffView({ diff }: { diff: string }) {
  if (!diff.trim()) {
    return <div className="text-xs text-muted-foreground">No diff.</div>;
  }
  return (
...
function ActivityView({ runs }: { runs: RunWithEvents[] }) {
  if (runs.length === 0) {
    return <div className="text-xs text-muted-foreground">No runs yet.</div>;
  }
  return (
...
function ReviewView({ review }: { review: Review | null }) {
  if (review === null) {
    return <div className="text-xs text-muted-foreground">No review yet.</div>;
  }
  return (
...
export function RightPanel({
  ...
  return (
    <div
      ...
      className={`absolute inset-y-0 right-0 z-20 flex flex-col border-l border-border bg-background shadow-2xl transition-transform duration-200 ${
      ...
      {open && (
        <div
          ...
          className={`absolute left-0 top-0 h-full w-1.5 cursor-col-resize transition-colors ${
            isResizing ? "bg-blue-500" : "bg-transparent hover:bg-secondary"
          }`}
        />
      )}
      {panel && (
